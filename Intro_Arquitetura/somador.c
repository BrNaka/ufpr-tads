...
char* somador (char* A, char* B, char* SUM) {
    int indx, indxSum, con;
    size_t a_numSize, b_numSize;
    char saida[SIZE], carryOut, carryIn;
    ...
    for (indxSum, indx; indxSum < a_numSize; indxSum++, indx--) {
        ...
        // Somador Completo
            ...
            carryIn = carryOut;
            saida[indxSum] = XOR(XOR(A[indx], B[indx]), carryIn);
            carryOut = OR(AND(XOR(A[indx], B[indx]), carryIn), AND(A[indx], B[indx]));
        }
    }

    saida[indxSum] = carryOut;

    for (indxSum, con; indxSum >= 0; indxSum--, con++) {
        SUM[con] = saida[indxSum];
    }
